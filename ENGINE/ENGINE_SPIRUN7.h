...
typedef struct SPIRUN7_GUI_TRACE_LOG_LIST_SHOW_INFORMATION
{
	...
	pHANHELD pLast__Called_HANHELD[SPURUN7_LIMIT_GUI_LIST_SHOW_LATEST_HANHELDS]; //указатели на pVMPX_VM_PROFILE_INFO_STACK.HANHELD
...
typedef struct SPIRUN7_GUI_POPULAR_LIST_SHOW_INFORMATION
{
	...
	pHANHELD pPopular_HANHELD[VM_HANDLES_MAX_COUNT]; //указатели на pVMPX_VM_PROFILE_INFO_STACK.HANHELD
...
extern pVMPX_VM_PROFILE_INFO_STACK DH_SPIRUN_Init_assign_vm_profile(__in TCHAR* assign_vm_name);//доп. инициализация перехватываемого профиля vm
extern bool DH_SPIRUN_Intrecept_Commander_Tower(
	...
	__in pVMPX_VM_PROFILE_INFO_STACK Selected_vmp_stack //vmp стек
	); //командный узел по перехвату

bool DH_SPIRUN_Sentry_for_HANHELD_ID_dumping_region(
	__in pVMPX_VM_PROFILE_INFO_STACK Selected_vmp_stack, //выбранный профиль виртуальной машины
...
SERVICE_TYPE DH_PROFILE_SPIRUN7_TRANSLATE_VMP2_TRACE_LOG_HANHELD_ID(
	...
	__in pVMPX_VM_PROFILE_INFO_STACK VMP2_Info_Stack, //профиль виртуальной машины
...
HANHELD_ADDRES DH_PROFILE_SPIRUN7_TRANSLATE_VMP2_TRACE_LOG_HANHELD_ID_Address_only(
	...
	__in pVMPX_VM_PROFILE_INFO_STACK VMP2_Info_Stack
...
pREGION_COPY __fastcall DH_SPIRUN7_Take_Region_Snapshot_by_addres(
	__in SERVICE_ADDRES RegionAddres, //RVA-адрес
	__in SERVICE_TYPE RegionType, //предлагаекмый тип реоигна
	__out PIMAGE_SECTION_HEADER* pPESection_header //ссылка на PE-секцию (если доступно)
	);//создание снапшота региона по адресу (пример использования - для анализатора контекстов виртуальной машины). Ищет по PE-секции